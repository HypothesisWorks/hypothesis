...
def test_fuzz_fractions_bounds(data):
    ...
    try:
        ...
        assume(False)  # fractions too close for given max_denominator
    if low is not None:
        assert low <= val
    if high is not None:
        assert val <= high
...
def test_fuzz_decimals_bounds(data):
    places = data.draw(none() | integers(0, 20))
    finite_decs = decimals(allow_nan=False, allow_infinity=False,
                           places=places) | none()
    low, high = data.draw(tuples(finite_decs, finite_decs))
    if low is not None and high is not None and low > high:
        low, high = high, low
    val = data.draw(decimals(low, high, allow_nan=False,
