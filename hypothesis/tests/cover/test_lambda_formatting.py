...
def test_wrappers_do_not_share_descriptions_with_each_other():
    # The wrappers generated by proxies() have identical code objects for any
    # given signature, so their descriptions must be keyed on the wrapped
    # function rather than on the wrapper itself.
    @proxies(lambda x: x)
    def one(*args, **kwargs):
        pass  # pragma: no cover

    @proxies(lambda x: None)
    def two(*args, **kwargs):
        pass  # pragma: no cover

    assert get_pretty_function_description(one) == "lambda x: x"
    assert get_pretty_function_description(two) == "lambda x: None"
