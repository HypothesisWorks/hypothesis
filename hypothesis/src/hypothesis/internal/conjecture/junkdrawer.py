...
def deep_equal(a: Any, b: Any) -> bool:
    """Like ==, but compares floats by bitpattern (so nan == nan, 0.0 != -0.0)
    and requires identical types (so True != 1), recursively in collections."""
    ...
    if isinstance(a, dict):
        # Because we need custom equality, dict and set comparisons turn from
        # O(n) to O(n^2). This is unfortunate and maybe not worth the cost.
        if len(a) != len(b):
            return False
