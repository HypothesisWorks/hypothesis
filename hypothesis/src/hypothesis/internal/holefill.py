...
def _coercions(value: Any) -> Iterator[ChoiceT]:
    ...
    for convert in (int, float, str):
        try:
            coerced = convert(value)  # deliberately duck-typed
        except Exception:
            continue
...
def _mutate_choice(value: ChoiceT, random: Random, hint: str) -> ChoiceT:
    ...
    if isinstance(value, float):
        if not math.isfinite(value):
            return 0.0
        fdelta = 2.0 ** random.randrange(-4, 10)
        return random.choice((value + fdelta, value - fdelta, value / 2, -value))
...
class _HoleSearch:
    ...
    def consider(self, prefix: tuple[ChoiceT | ValueHole, ...] = ()) -> None:
        ...
        if key in self.seen:
            return
        self.seen.add(key)
        if equal_values(value, self.record.value):
            distance = 0.0
        else:
