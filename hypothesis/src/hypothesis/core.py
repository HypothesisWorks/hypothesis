...
class example:
    ...
    explicit inputs added by |@example| do not shrink, unless marked with
    |@example.shrink|. If an explicit input fails, Hypothesis will stop and
    report the failure without generating any random inputs.
    ...
    def xfail(
        ...
        *,
        ...
        if condition:
            if self._this_example.shrink:
                raise InvalidArgument(
                    "Cannot combine .xfail() with .shrink(): shrinking searches "
                    "for a minimal failing example to report as a test failure, "
                    "while xfail expects the failure and reports success."
                )
    ...
    def shrink(self) -> "example":
        """If this explicit example fails, shrink it before reporting.

        This method is provisional: we expect to replace it with a
        :obj:`~hypothesis.Phase` controlling shrinking of explicit examples
        in a future release, and may change or remove it without deprecation.

        Explicit examples do not normally shrink, because Hypothesis usually
        cannot tell which choices would have generated them. With ``.shrink()``,
        when the example fails, Hypothesis instead re-encodes each argument as
        if it had been generated by the corresponding strategy, confirms that
        the re-encoded input reproduces the failure, and then shrinks and
        reports the minimal failing example - so a large pasted regression
        input yields the same succinct report as a generated failure.
        ...
        """
        if self._this_example.raises is not None:
            raise InvalidArgument(
                "Cannot combine .shrink() with .xfail(): shrinking searches "
                "for a minimal failing example to report as a test failure, "
                "while xfail expects the failure and reports success."
            )
        self._this_example = dataclasses.replace(self._this_example, shrink=True)
        return self
...
def _reportable_failure(state, runner, result):
    ...
    ran_example = runner.new_conjecture_data(
        result.choices, max_choices=len(result.choices)
    )
    ran_example.span_comments = result.span_comments
    try:
        ...
        # execute_once() raises the expected error or FlakyFailure, so
        # completing normally should be unreachable; fall back regardless.
        return None
    except (StopTest, UnsatisfiedAssumption, FlakyFailure):
        return None
    except BaseException as e:
        if state.settings.print_blob:
            fragments.append(
                "\nYou can reproduce this example by temporarily adding "
                f"{reproduction_decorator(result.choices)} "
                "as a decorator on your test case"
            )
        return ReportableError(fragments, e.with_traceback(get_trimmed_traceback()))
    finally:
        ran_example.freeze()


def _shrink_explicit_example(state, example_kwargs, err):
    ...
    runner = ConjectureRunner(
        ...
        # Replaying and shrinking operate on concrete choices, so always run
        # them on the standard backend, whatever the test itself uses.
        settings=Settings(parent=state.settings, backend="hypothesis"),
        random=state.random,
        database_key=None,
        thread_overlap=state.thread_overlap,
    )
    # Require the same kind of failure at the same location. The full
    # InterestingOrigin may differ in its __context__ chain between the
    # explicit-example and engine execution paths, so don't compare that.
    expected = InterestingOrigin.from_exception(err)

    def matches(origin):
        return (origin.exc_type, origin.filename, origin.lineno) == (
            expected.exc_type,
            expected.filename,
            expected.lineno,
        )
    ...
    others = [
        data
        for origin, data in runner.interesting_examples.items()
        if not matches(origin)
    ]
    shrunk = (
        _reportable_failure(
            state, runner, min(matching, key=lambda d: sort_key(d.nodes))
        )
        if matching
        else None
    )
    extras = [
        reportable
        for data in sorted(others, key=lambda d: sort_key(d.nodes))
        if (reportable := _reportable_failure(state, runner, data)) is not None
    ]
    return shrunk, extras
    ...
    for example in reversed(getattr(wrapped_test, "hypothesis_explicit_examples", ())):
        ...
        with local_settings(state.settings):
            ...
            try:
                ...
                with contextlib.suppress(StopTest):
                    empty_data.conclude_test(Status.INVALID)
                reportable = ReportableError(fragments_reported, err)
                extra_reportables = []
                if (
                    example.shrink
                    ...
                    # @example(...).shrink(): report the shrunk failing example
                    # instead, if the failure can be re-encoded and reproduced.
                    shrunk, extra_reportables = _shrink_explicit_example(
                        state, example_kwargs, err
                    )
                    reportable = shrunk or reportable
                yield reportable
                if (
                    ...
                    # other failures surfaced while shrinking an explicit example
                    yield from extra_reportables
                    continue
                break
            finally:
