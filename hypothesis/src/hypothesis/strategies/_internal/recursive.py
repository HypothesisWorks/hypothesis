...
class LimitedStrategy(SearchStrategy[T]):
    ...
    def _invert(self, value: Any) -> InvertResultT:
        # The marker is not part of the choice sequence, so at the choice
        # level a LimitedStrategy draw is exactly a base_strategy draw. If a
        # replay of the inversion exhausts the marker it raises LimitReached
        # partway through the choices, and the caller's replay-verification
        # rejects the attempt - as inversion's best-effort contract permits.
...
class RecursiveStrategy(SearchStrategy):
    ...
    def _invert(self, value: Any) -> InvertResultT:
        # do_draw makes exactly one draw from self.strategy per attempt; its
        # retry paths (LimitReached, min_leaves unsatisfied) only replay under
        # a bad inversion, which the caller's replay-verification rejects.
