...
def _str_to_path(s, result_type):
    ...
    if isinstance(s, bytes) and result_type is text_type:  # pragma: no cover
...
def _path_root(draw, result_type):
    ...
    if os.name != 'nt':
        ...
        network = one_of([
            ...
            builds(lambda *x: tp().join(x),
                   sep, sep, just(tp('?')), sep, just(tp('UNC')), sep, name,
                   sep, name, sep),
            # \\.\[physical_device]\
            builds(lambda *x: tp().join(x),
        ...
        return draw(one_of(relative, drive, extended, network))
...
def fspaths(draw, allow_pathlike=None, allow_existing=False):
    ...
    if allow_pathlike and not has_pathlike:  # pragma: no cover
        raise InvalidArgument(
...
assert set(s for s in _strategies if not s.startswith('_')).issubset(
    set(__all__)), _strategies - set(__all__)
