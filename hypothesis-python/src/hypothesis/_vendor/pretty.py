...
from hypothesis._internal.compat import PY3, cast_unicode, string_types, \
    get_stream_enc
...
class RepresentationPrinter(PrettyPrinter):
    ...
    def __init__(self, output, verbose=False, max_width=79, newline='\n',
        ...
        PrettyPrinter.__init__(self, output, max_width,
                               newline, max_seq_length=max_seq_length)
...
def _seq_pprinter_factory(start, end, basetype):
    ...
    def inner(obj, p, cycle):
        typ = type(obj)
        if (
            basetype is not None and typ is not basetype and
            typ.__repr__ != basetype.__repr__
        ):
