...
# whose generated value has one of these exact types gets its span annotated
# with that value. We use a tuple rather than a set because ``in`` on a tuple
# relies only on equality, so it still works if the generated value's class has
# an unhashable metaclass.
...
class Span:
    ...
    def annotation(self) -> Any:
        """The value annotating this span, or ``None`` if there is none.
        Spans produced by a strategy generating a primitive value (one of the
        types we have a choice node for - ``int``, ``bool``, ``str``, ``bytes``,
        or ``float``) are annotated with the generated value."""
        return self.owner.annotations.get(self.index)
...
class SpanRecord:
    ...
    def annotate_current_span(self, value: Any) -> None:
        # Attach a value to the most recently started span. Called by
        # ConjectureData.draw to record the value generated by a strategy.
        assert self.__open_spans, "Cannot annotate without an open span"
        self.annotations[self.__open_spans[-1]] = value
...
class Spans:
    ...
    def __init__(self, record: SpanRecord) -> None:
        ...
        self.annotations: dict[int, Any] = record.annotations
...
class ConjectureData:
    ...
    def maybe_add_choice_node_for(self, value: Any) -> None:
        ...
        value is visible to span-level machinery (annotations, shrinking
        widenings). For non-primitive values it is a no-op.
        """
    ...
    def draw(
        ...
        try:
            ...
            if type(v) in _PRIMITIVE_CHOICE_TYPES:
                self.__span_record.annotate_current_span(v)
            return v
        finally:
            self.stop_span()
